...
TNFAFinalStateIndel::TNFAFinalStateIndel(int len, int edits)
  : TNFAStateIndel(0), patternLength_(len), maxEdits_(edits) {}

void TNFAFinalStateIndel::addOutStates(bool,
                                       string::const_iterator,
                                       vector< TNFAStateIndel * > [],
                                       unordered_map<int, int> &,
                                       uint32_t) {}

void TNFAFinalStateIndel::addEpsilonTransitions(bool listNo,
                                                string::const_iterator,
                                                vector< TNFAStateIndel * >[],
                                                ...
                                                uint32_t)
{
  ...
  for (int c = kErrorCodeBits -1; c >= 0; c--) {
    ...
    if (errorCode_[listNo] & (uint64_t) 1 << c) {
      ...
      editsLeft = counterToMismatches(c) + counterToIndels(c);
