...
#ifndef SEQSCAN_PU_TNFA_STATE_EDITS_H_
#define SEQSCAN_PU_TNFA_STATE_EDITS_H_
...
class TNFAStateEdits {
 public:
  ...
  void setOutPtr(TNFAStateEdits *);  // Set pointer to outgoing state
  TNFAStateEdits *getOutPtr();       // Get pointer to outgoing state
  ...
  bool edits(uint64_t);  // Are edits allowed
  uint64_t decrementEdits(uint64_t);  // Decr. edits in error code
  static inline int counterToEditss( int cnt ) { return cnt; }
protected:
  char c;  // Character to be matched
  TNFAStateEdits *out_;  // Outgoing state
...
#endif  // SEQSCAN_PU_TNFA_STATE_EDITS_H_
